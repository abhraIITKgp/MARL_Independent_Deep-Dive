...
import numpy as np
from wrappers import Dict2TupleWrapper
import supersuit
import torch
...
class QMix_Trainer():
    ...
    def update(self, batch_size, situation_1='max', situation_2='max'):
        ...
        fun_target_agent_1 = getattr(target_agent_outs, situation_1)
        fun_target_agent_2 = getattr(target_agent_outs, situation_2)
        target_qvals_1 = fun_target_agent_1(dim=-1, keepdim=True)[0] # [#batch, #sequence, #agents, action_shape]
        target_qvals_2 = fun_target_agent_2(dim=-1, keepdim=True)[0] # [#batch, #sequence, #agents, action_shape]
        target_qvals_1 = target_qvals_1[:,:,0,:,:]
        target_qvals_2 = target_qvals_2[:,:,1,:,:]
        target_qvals = torch.stack((target_qvals_1, target_qvals_2), dim=2)
        target_qtot = self.target_mixer(target_qvals, next_state)
...
def main():  
    ...
    parser.add_argument('-s', '--situation', nargs='+', default=['max', 'max'])
    args = parser.parse_args()
    
    if args.train and len(args.situation) < 2:
        raise ValueError(f'Not enough situations, received {args.situation}')
    ...
    learner = QMix_Trainer(replay_buffer, 
                            n_agents, 
                            state_dim, 
                            action_shape, 
                            action_dim, 
    ...
    if args.test:
        ...
        print(f'Testing for {args.max_episodes} episodes in situation {" ".join(args.situation)}...')
    else:
        print(f'Training for {args.max_episodes} episodes in situation {" ".join(args.situation)}...')
        
    loss = None
    wins = np.zeros(max_episodes)
    ...
    for epi in tqdm(range(max_episodes)):
        ...
        if np.sum(episode_reward) > BASE_LINE:
                wins[epi] = 1
        ...
        if args.train:
            ...
            if epi > batch_size:
                for _ in range(update_iter):
                    loss = learner.update(batch_size, args.situation[0], args.situation[1])

            if epi % save_interval == 0 or epi == max_episodes - 1:
        ...
        if args.test and args.verbose:
            print(f'Episode: {epi} | Episode Reward: {np.sum(episode_reward)}')
    
    print(f'Win rates: {len(wins[wins > 0]) / len(wins)}')
